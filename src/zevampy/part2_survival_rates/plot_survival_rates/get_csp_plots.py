...
def get_csp_plots(survival_rates, fitted_csp_values, config_all, config_group, survival_grouping, powertrains, csp_plot_years):
    """
    Generate CSP plots for all survival-rate groups.
    ...
    Returns:
        None
    """
    # Keep only config.yaml powertrains
    ...
    survival_rates = survival_rates[
        survival_rates[stock_year_dim].isin(csp_plot_years)
    ].copy()
    fitted_csp_values = fitted_csp_values[
        fitted_csp_values[stock_year_dim].isin(csp_plot_years)
    ].copy()
