...
def test_registration_start_year_is_derived_from_stock_horizon():
    """The earliest cohort year should be derived instead of configured explicitly."""
    _, inputs = load_data_and_prepare_inputs(INPUT_DIR, config=deepcopy(BASE_CONFIG))
...
def test_all_three_survival_sources_run_and_previous_outputs_can_be_reused(tmp_path):
    ...
    empirical_config["survival_rates"] = {
        ...
        "files": {
            "empirical": str(
                (baseline_dir / "2_2_empirical_survival_rates.csv").resolve()
            )
        },
    }
    ...
    parameter_config["survival_rates"] = {
        ...
        "files": {
            "parameters": str(
                (baseline_dir / "2_1_optimum_parameters_csp_curves.csv").resolve()
            )
        },
    }
...
def test_parameters_source_does_not_require_stock_by_age_or_stock_year(tmp_path):
    ...
    config["survival_rates"] = {
        ...
        "grouping": ["geo country"],
        "csp_available_years": 45,
        "files": {"parameters": "alternative_csp_parameters.csv"},
    }
...
def test_alternative_survival_source_requires_file(tmp_path):
    ...
    with pytest.raises(ValueError, match="survival_rates.files.empirical"):
        load_data_and_prepare_inputs(INPUT_DIR, config=config)
...
def test_missing_alternative_survival_file_raises_clear_error(tmp_path):
    ...
    config["survival_rates"] = {
        ...
        "grouping": ["geo country"],
        "csp_available_years": 45,
        "files": {"parameters": "file_that_does_not_exist.csv"},
    }
    config["data"]["output_path"] = str(tmp_path / "outputs")
